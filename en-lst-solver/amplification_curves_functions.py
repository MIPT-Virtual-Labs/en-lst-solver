...
class ACS_InputParameters(BaseModel):
    omega_min: int
    omega_max: int
    ...
    def check_Re(cls, number_of_omegas):
        if number_of_omegas<1 or number_of_omegas>8  :
            raise ValueError("Number of omegas is out of range. Must be in [1,8]")
        return number_of_omegas

def amplfication_curves_solve(p: ACS_InputParameters) -> np.ndarray:
    # mesh
    #N = 800
    ...
    for omega in omega_mesh:#pbar_omega(omega_mesh):
        ai_s = []
        for x in x_mesh:
    ...
    return {'x_mesh' : x_mesh, 
            'ai_for_omega' : ai_for_omega, 
